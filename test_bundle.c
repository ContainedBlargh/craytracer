...
Scene *parse_scene(char* path)
{
    ...
    struct json_value_s *root = json_parse(contents, strlen(contents));
    ...
    for (u32 i = 0; i < obj->length; i++) {
        if (strncasecmp("camera", current->name->string, current->name->string_size) == 0) {
            Vec3 position;
            Vec3 direction;
            ...
            for (u32 j = 0; j < obj->length; j++) {
                ...
                struct json_object_s *position_obj = json_value_as_object(current_inner->value);
                struct json_object_element_s *pos_current = position_obj->start;
                for (u32 k = 0; k < obj->length; k++) {
                    if (strncasecmp("x", pos_current->name->string, pos_current->name->string_size) == 0) {
                        struct json_number_s *x = json_value_as_number(pos_current->value);
                        if (x == NULL) {
                            failwithf("Parser error, expected to be a number, but got the type '%u'\n", pos_current->value->type);
                        }
                        vector->x = atof(x->number);
                    } else if (strncasecmp("y", pos_current->name->string, pos_current->name->string_size) == 0) {
                        struct json_number_s *y = json_value_as_number(pos_current->value);
                        if (y == NULL) {
                            failwithf("Parser error, expected to be a number, but got the type '%u'\n", pos_current->value->type);
                        }
                        vector->y = atof(y->number);
                    } else if (strncasecmp("z", pos_current->name->string, pos_current->name->string_size) == 0) {
                        struct json_number_s *z = json_value_as_number(pos_current->value);
                        if (z == NULL) {
                            failwithf("Parser error, expected to be a number, but got the type '%u'\n", pos_current->value->type);
                        }
                        vector->z = atof(z->number);
                    }
...
#pragma endregion ___SRC_PARSER_H

#pragma region PARSER_TEST_C
...
#pragma endregion PARSER_TEST_C
